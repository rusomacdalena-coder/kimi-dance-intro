...
function extractSummary(analysisText, title) {
  const paragraph = analysisText
    .split(/\n{2,}/)
    .map((block) => stripMarkdown(block.replace(/\n/g, ' ')))
    .find((block) => block && !block.startsWith('---') && block.length >= 24)
  return truncate(
    paragraph || `${title} 的 AI 拉片案例，公开叙事结构、节奏数据、情绪/切镜统计和分镜表摘要，不公开完整剧本正文。`,
    150,
  )
}
...
function buildMetrics(rows) {
  ...
  const sceneCount = new Set(rows.map((row) => row['场景ID']).filter(Boolean)).size
  const emotionTop = countBy(rows, '宏情绪')[0] || countBy(rows, '情绪')[0]
  const narrativeTop = countBy(rows, '叙事功能')[0]

  const metrics = []
  if (shotCount) metrics.push({ label: '镜头数', value: String(shotCount), note: '来自 breakdown.csv' })
  if (duration) metrics.push({ label: '视频时长', value: formatDuration(duration), note: '按镜头时长汇总' })
  if (sceneCount) metrics.push({ label: '场景数', value: String(sceneCount), note: '按场景ID去重' })
  if (emotionTop) metrics.push({ label: '主要情绪', value: emotionTop[0], note: `${emotionTop[1]} 个镜头` })
  if (narrativeTop) metrics.push({ label: '最高频叙事功能', value: narrativeTop[0], note: `${narrativeTop[1]} 个镜头` })
...
function buildMethodology(rows, analysisText) {
  ...
  if (narrativeTop.length) methodology.push(`高频叙事功能集中在：${narrativeTop.map(([name, count]) => `${name}（${count}）`).join('、')}。`)
  if (emotionTop.length) methodology.push(`宏情绪分布以 ${emotionTop.map(([name, count]) => `${name}（${count}）`).join('、')} 为主。`)
