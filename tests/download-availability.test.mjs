...
test('download page publishes the verified macOS 0.4.13 release', async () => {
  ...
  for (const content of [page, built]) {
    // exact authorized bytes: sha256 for Mac 0.4.13 (server sha256sum, 2026-09-17)
    assert.match(
      content,
      /https:\/\/api\.kimidance\.com\/downloads\/Kimidance-0\.4\.13-macOS\.dmg/,
    )
    assert.match(content, /67f85a76bfbc8b635ee2f45c8099e1ce609791fde597650f1a52587c7d57d925/)
    assert.match(content, /下载 macOS 版 0\.4\.13/)
    ...
    // only the authorized 0.4.13 DMG may be served from api.kimidance.com
    assert.doesNotMatch(
      content,
      /https:\/\/api\.kimidance\.com\/downloads\/(?!Kimidance-0\.4\.13-macOS\.dmg)[^"' ]+\.dmg/,
    )
    ...
    // only the authorized 0.4.13-r1 installer may be served from api.kimidance.com
    assert.doesNotMatch(
      content,
      /https:\/\/api\.kimidance\.com\/downloads\/(?!Kimidance-Windows-Setup-0\.4\.13-r1-x64\.exe)[^"' ]+\.(?:exe|msi)/,
    )
  }
})

test('download page publishes the authorized Windows 0.4.13-r1 release', async () => {
  const page = await readFile(new URL('../src/pages/download.astro', import.meta.url), 'utf8')
  const built = await readFile(new URL('../docs/download/index.html', import.meta.url), 'utf8')

  for (const content of [page, built]) {
    // exact authorized bytes: sha256 for Windows 0.4.13-r1 (server sha256sum, 2026-09-18)
    assert.match(content, /9ceedddf73c07ef341d64d18676b7cc8d07511ec26f78adb5ca5216cd725cfc7/)
    assert.match(content, /Kimidance-Windows-Setup-0\.4\.13-r1-x64\.exe/)
    assert.match(content, /下载 Windows 版 0\.4\.13/)
    // superseded 0.4.11-r16 Windows artifacts must not resurface
    assert.doesNotMatch(content, /Kimidance-Windows-Setup-0\.4\.11-r16-x64\.exe/)
    assert.doesNotMatch(content, /66735b16d67c7b42ef2514ee34e287a80cb052dd16cc7a7d5594b9d4807c6118/)
    ...
    assert.match(
      content,
      /https:\/\/api\.kimidance\.com\/downloads\/Kimidance-Windows-Setup-0\.4\.13-r1-x64\.exe/,
    )
