...
## 8. 布局调整(f262dc9,应 Oliver 反馈)

hero 内嵌白卡被反馈「太突兀」。调整:影片移出 hero,改为紧跟第一屏的**独立全宽横栏**——通栏 #f3f2f2 纸白、无卡片边框/阴影,影片 1152px 居中,读作横贯暗页的印刷样张带;hero 恢复原始紧凑排版(高度 1163→828px)。预水合缩放阶梯按 1152px 盒重算。线上 21:45:42 生效(25 秒);桌面/手机零溢出复验,套件 6/6 绿。

## 9. 墨水版重绘(5b6f659,K3 研究后)

横栏纸白仍被反馈与暖暗站冲突。按 Oliver 指示研究 Kimi K3 的融入手法:moonshot.ai(纯黑站)首页动画与 K3/K2.6 卡片视频全部黑底单色光效,kimi.ai K3 博客(白站)头图为纸白底单色墨迹——**两站同一铁律:动画画布 = 页面底色家族,对比只住在笔画里,绝不做异色板**。

应用:影片作用域 token 全量重映射到 global.css 站点色(bg `hsl(20 10% 5%)` == 页面,实测 `filmBg==pageBg==rgb(14,12,11)`;奶油 ink、暖中性层);**CMYK 相乘分色只在白底成立**,墨水版换成互斥 R/G/B 三版 + screen 混合(套准时并集精确复原照片,失准时为暗底 RGB 光晕分离;K 版隐藏防双重曝光,CSS 重指滤镜,影片版结构零改动);scan/grain/slam 三处 multiply→screen;TRUST 深色块自动反转为全片唯一亮拍(块内标签走 `--lq-onvoid`)。纸白版完整参数留在 hero.css 注释,一步可回。线上 22:01:42 生效(24 秒);零溢出、6/6 绿。
